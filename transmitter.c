...
// after this start with the command:
//   sudo ./transmitter [song] [freq] [volume]
...
void setup_fm(int state) {
    ...
    if (allof7e == (unsigned char *) -1) exit(-1);
    struct GPFSEL0_T {
        char FSEL0 : 3;
        char FSEL1 : 3;
        char FSEL2 : 3;
...
int main(int argc, char **argv) {
    if (argc > 1) {
        ...
        atexit(&shutdown_fm);
        setup_fm(1);
        float freq_out = argc > 2 ? atof(argv[2]) : 77.7;
        float bandwidth = argc > 3 ? atof(argv[3]) : 8;
        int freq_pi = 500;
        int mod = (freq_pi / freq_out) * 4096;
        modulate(mod);
        playWav(argv[1], mod, bandwidth);
    } else {
        fprintf(stderr,"ERROR!!!! START WITH ARGUMENTS: [song name].wav [freq(float)] \n");
    }
    return 0;
}
